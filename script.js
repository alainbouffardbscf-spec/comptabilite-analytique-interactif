...
function mettreAJour() {
  ...
  const calculs = calculerMontantsSelonTypeExercice(bv);
...
function calculerMontantsSelonTypeExercice(bv) {
  const typeExercice = obtenirTypeExercice();

  if (typeExercice === "fabrication-continue") {
    return calculerMontantsFabricationContinue(bv);
  }
...
function calculerMontantsFabricationContinue(bv) {
  ...
  (exerciceData.Ecritures || []).forEach(ecriture => {
    ...
    const estCochee = obtenirEcrituresCochees().includes(ecriture.IDEcriture);
    if (!estCochee) return;
    ...
    if (lignes.some(l => Number(l.CompteID) === 5007)) {
...
function afficherBilan(bv, calculs) {
  ...
  const lignesBilan = bv
    .map(ligne => {
      ...
      const inclureDansBilan =
        compte.Etat === "Bilan" ||
        (typeExercice === "prix-revient-commande" && Number(ligne.CompteID) === 9640);

      if (!inclureDansBilan) return null;

      const soldeNet = Number(ligne.Debit) - Number(ligne.Credit);

      if (typeExercice === "prix-revient-commande" && Number(ligne.CompteID) === 9640) {
        return {
          Compte: "FGF imputés",
          Section: "Passif",
          OrdreID: "10209640",
